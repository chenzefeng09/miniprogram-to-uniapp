...
function templateTagHandle () {
    ...
    for (const key in templateList) {
        ///wxparse貌似会一直循环，很久。。。。
        if (!/wxParse|WxEmojiView/i.test(key)) {
    ...
    for (let i = tagList.length - 1; i >= 0; i--) {
        ...
        if (/wxParse|WxEmojiView/i.test(templateName)) {
            tagList.splice(i, 1);;
        }
    }
    ...
    for (const key in tagList) {
        ...
        if (!/wxParse|WxEmojiView/i.test(templateName) && pagesData[fileKey] && pagesData[fileKey].data) {
            let templateWxml = "";
            if (/\?/.test(templateName)) {
                //含有三元表达式的情况，需注意，目前仅支持简单的三元表达式
                let reg = /{{\s*(.*?)\s*\?\s*(.*?)\s*:\s*(.*?)\s*}}/;
